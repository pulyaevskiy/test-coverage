...
String getPackageName(Directory packageRoot) {
  final yamlPath = path.join(packageRoot.absolute.path, 'pubspec.yaml');
  final yamlData = File(yamlPath).readAsStringSync();
  var yaml = Pubspec.parse(yamlData, lenient: true);
  return yaml.name;
}

List<File> findSourceFiles(Directory packageRoot, {Glob excludeGlob}) {
  final srcsPath = path.join(packageRoot.absolute.path, 'lib');
  final srcsRoot = Directory(srcsPath);
  final contents = srcsRoot.listSync(recursive: true);
  final result = <File>[];
  for (final item in contents) {
    if (item is! File) continue;
    if (!item.path.endsWith('.dart')) continue;
...
void generateMainScript(Directory packageRoot, List<File> testFiles,
  ...
  final nSrcFiles = sourceFiles == null ? 0 : sourceFiles.length;
  ...
  if (nSrcFiles > 0) {
    final packageName = getPackageName(packageRoot);
    final regex = RegExp(r'^lib/');
    for (final source in sourceFiles) {
      final rpath = path
          .relative(source.path, from: packageRoot.path)
          .replaceFirst(regex, '');
      imports.add('import \'package:$packageName/$rpath\';');
    }
  }
  imports.sort();
  ...
  if (nSrcFiles > 0) buffer.writeln('// ignore_for_file: unused_import');
